...
var immutableSet = function immutableSet(object, path) {
  ...
  var childObject = {};
  if (lodash.has(object, pathSplit[0])) {
    childObject = object[pathSplit[0]];
    if (!lodash.isPlainObject(childObject)) {
  ...
  return _extends({}, object, defineProperty({}, pathSplit[0], immutableSet(childObject, lodash.tail(pathSplit).join(delimiter), value, delimiter)));
};
